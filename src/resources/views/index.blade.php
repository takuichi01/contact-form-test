    ...
    <form class="form" action="{{ route('contact.confirm') }}" method="post">
        ...
        <div class="form__row">
            ...
            <select class="form__select" name="category" required>
                ...
                <option value="{{ $category->content }}" @if (($data['category'] ?? '' )===$category->content) selected @endif>
                    {{ $category->content }}
                </option>
                @endforeach
            </select>
        ...
        <div class="form__row">
            ...
            <textarea class="form__textarea" rows="5" name="detail"
                placeholder="お問い合わせ内容をご記載ください">{{ $data['detail'] ?? ''}}</textarea>
        </div>
